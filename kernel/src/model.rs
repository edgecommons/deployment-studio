...
//!
//! Mirrors `schema/deployment-definition.schema.json`. Field-level semantics live in
//! `schema/DEFINITION.md`; the semantic rules S-1..S-9 are enforced by [`crate::validate`].
...
impl Node {
    /// The platform identity bound to the node key (defaults to the key itself).
