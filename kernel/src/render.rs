//! The HOST renderer (slice 1): compiles a DeploymentDefinition + an environment's bindings into
//! the per-node native artifacts — the ConfigComponent catalog, the rendered bootstrap config,
//! the messaging files, and the supervisord conf — plus the workspace-level `plan.json` and
//! `requirements.json` of the three-file IaC handshake.
//!
//! Determinism is a contract: no timestamps, no randomness; identical inputs yield identical
//! bytes. JSON is emitted with 2-space pretty-printing and a trailing newline.

use anyhow::{Context as _, Result};
use serde_json::{json, Map, Value};

use crate::context::{collect_tokens, resolve_tokens, Workspace};
...
/// Restart impact classification per config source (a property of the source, not the platform).
pub fn restart_impact(config_source: &str) -> &'static str {
    match config_source {
        "CONFIG_COMPONENT" | "SHADOW" => "hot-push",
        "FILE" | "CONFIGMAP" => "hot-watch",
        _ => "restart",
    }
}

pub fn render(ws: &Workspace, environment: &str, release_tag: &str) -> Result<RenderOutput> {
    ...
    for node in &ws.definition.nodes {
        ...
        // ---- Load + resolve this node's layer set.
        let mut scope_layers: Vec<(String, Map<String, Value>)> = Vec::new(); // (scope id, content)
        for scope in &chain {
            let mut content = match &scope.layer {
                Some(rel) => Value::Object(ws.load_layer(rel)?),
                None => Value::Object(Map::new()),
            };
            collect_tokens(&content, "binding", &mut all_tokens);
            resolve_tokens(&mut content, "binding", &bindings)
                .with_context(|| format!("scope {}", scope.id))?;
            match content {
        ...
        // ---- Per-node catalog (when the node has a config provider).
        if let Some(provider) = &node.config_provider {
            ...
            for (idx, (scope_id, layer)) in scope_layers.iter().enumerate() {
                ...
                // Cumulative scope map, in level order.
                ...
                // Node config: [hierarchy (root only)] + own-level identity claim + layer content.
            ...
            // ---- Rendered bootstrap: chain merge + provider overlay, derived keys stamped first.
        ...
        // ---- Plan entry.
        plan_nodes.push(json!({
            "key": node.key,
            "thingName": node.thing_name(),
            "scope": node.scope,
            "levels": levels,
            "catalogVersion": node.config_provider.as_ref().map(|_| &catalog_version),
            "components": node.components.iter().map(|c| json!({
                "name": c.name,
                "configSource": c.config_source,
                "restartImpact": restart_impact(&c.config_source),
                "artifact": c.artifact.as_ref().map(|a| json!({
                    "version": a.version,
                    "digest": a.digest,
                    "source": a.source.as_ref().map(|s| json!({
                        "kind": s.kind, "repo": s.repo, "features": s.features,
                    })),
                })),
                "files": c.files.iter().map(|f| json!({ "from": f.from, "to": f.to })).collect::<Vec<_>>(),
            })).collect::<Vec<_>>(),
        }));
    }

    // ---- Workspace-level handshake files.
    all_tokens.sort();
    all_tokens.dedup();
    files.push(RenderedFile {
    ...
    files.push(RenderedFile {
        ...
        text: pretty(&json!({
            ...
            "targetStandard": ws.definition.target_standard.family,
            "configRelease": release_tag,
            "nodes": plan_nodes,
        })),
    });
...
fn launch_command(
    ...
    let source_arg = match config_source {
        "FILE" => " /config/config-component-config.json".to_string(),
        _ => String::new(),
    };
    let base = format!(
        "{exec} --platform HOST --transport MQTT /config/{messaging_file} -c {config_source}{source_arg} -t {thing}"
    );
...
fn supervisord_conf(node: &Node) -> String {
    ...
    if let Some(broker) = &node.local_broker {
        ...
        let mut program =
            program_from_launch(&broker.kind, command, broker.launch.as_ref(), 10);
